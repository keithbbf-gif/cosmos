...
def _cli(root, *args):
    ...
    cosmos_py = here / "cosmos.py"
    if not cosmos_py.is_file():                       # repo layout: ../cosmos/cosmos.py
        cosmos_py = here.parent / "cosmos" / "cosmos.py"
    env = os.environ.copy()
    # PREPEND, never replace: cosmos_kernel imports cosmos_paths/lock/mail, which may
    # live on the caller's PYTHONPATH (the SPIKE layout) rather than beside cosmos.py.
    env["PYTHONPATH"] = os.pathsep.join(
        [str(cosmos_py.parent)]
